...
var logincommon = {
    ...
    _msalInstance: null,
    get msalInstance() {
        if (!logincommon._msalInstance)
            logincommon._msalInstance = new msal.PublicClientApplication(logincommon.msalConfig);
...
logincommon.login = function () {
    try {
        ...
        logincommon.msalInstance.handleRedirectPromise().then(logincommon.msalredirecthander, function (err) { logger.log('logn redirect error: ' + JSON.stringify(err)); });
...
logincommon.auth = async function () {
    try {
        ...
        if (allacocunts && allacocunts.length > 0)
            currentAccount = allacocunts[0];
        else
            currentAccount = null;
